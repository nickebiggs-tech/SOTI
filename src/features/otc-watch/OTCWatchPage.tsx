...
export function OTCWatchPage() {
  ...
  const watchData = useMemo(() => {
    ...
    const topRisers = [...allItems].filter(s => s[chgKey] > 0 && s.lyValue > 2000).sort((a, b) => b[chgKey] - a[chgKey]).slice(0, 15)
    const topDecliners = [...allItems].filter(s => s[chgKey] < 0 && s.lyValue > 2000).sort((a, b) => a[chgKey] - b[chgKey]).slice(0, 15)
  ...
  }, [state.otc, otcCategories, isValue])
  ...
  const waterfallData = useMemo(() => {
    ...
    const sorted = [...watchData.categories].filter(c => c.lyValue > 5000).sort((a, b) => chg(b) - chg(a))
    const topGainers = sorted.filter(c => chg(c) > 0).slice(0, 5).map(c => ({
      name: c.category,
      gain: chg(c),
      loss: 0,
    }))
    const topLosers = sorted.filter(c => chg(c) < 0).slice(-5).reverse().map(c => ({
      name: c.category,
      gain: 0,
      loss: chg(c),
    }))
  ...
  return (
    <div className="space-y-4 sm:space-y-6 page-enter">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 stagger-children">
        <KPICard title="OTC Market" value={isValue ? formatCompactDollar(otcTotalTY) : formatCompact(state.otc.reduce((s, r) => s + r.tyUnits, 0))} delta={otcGrowth} deltaLabel="YoY" icon={<ShoppingBag className="w-4 h-4" />} />
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden animate-fade-in-up">
        <div className="px-3 sm:px-5 py-3 border-b border-slate-100 bg-gradient-to-r from-teal-50/60 to-emerald-50/40">
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <h3 className="text-[11px] sm:text-xs font-bold text-slate-800">{isValue ? 'Value' : 'Volume'} Migration</h3>
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden animate-fade-in-up" style={{ animationDelay: '250ms' }}>
          <div className="px-3 sm:px-5 py-3 border-b border-slate-100 bg-gradient-to-r from-red-50/60 to-rose-50/40">
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <h3 className="text-[11px] sm:text-xs font-bold text-red-800">{isValue ? 'Value' : 'Volume'} at Risk</h3>
              ...
              {watchData.newEntrants.map(n => (
                <button key={n.sku} onClick={() => navigate('/otc', { state: { selectedCategory: n.category } })} className="bg-teal-50/40 rounded-lg border border-teal-100 p-3 hover:border-teal-300 hover:shadow-sm transition-all text-left group">
                  ...
                  <div className="flex items-center justify-between mt-2 pt-1.5 border-t border-teal-100">
                    <span className="text-[10px] font-bold text-teal-600">{fmt(isValue ? n.tyValue : n.tyUnits)}</span>
              ...
              <div className="px-4 sm:px-5 py-3 bg-gradient-to-r from-teal-50/30 to-emerald-50/20 border-b border-slate-100">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-[10px] text-slate-600 leading-relaxed">
                    ...
                    {(isValue ? catDetail.absChange : (catDetail.tyUnits - catDetail.lyUnits)) >= 0
                    ...
                    <div className="flex flex-wrap gap-2">
                      {catDetail.newProducts.map(n => (
                        <div key={n.sku} className="bg-white rounded-lg border border-teal-100 px-2.5 py-1.5 text-left">
                          ...
                          <p className="text-[8px] text-teal-600 font-bold">{fmt(isValue ? n.tyValue : n.tyUnits)} <span className="text-slate-400 font-normal">· {n.manufacturer}</span></p>
                        </div>
                      ))}
                    </div>
                  </div>
